        ...
        {activeTab === 'stack' && (
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {SKILL_CATEGORIES.map((cat, idx) => (
              <div
                key={idx}
                className="p-6 rounded-2xl bg-[#0c0e14] border border-white/[0.07] hover:border-[#09C8FF]/30 transition-all space-y-4 shadow-md"
              >
                ...
                <div className="space-y-2.5">
                  {cat.items.map((skill, sIdx) => (
                    <div
                      key={sIdx}
                      className="flex items-center justify-between p-2 rounded-lg bg-zinc-900/50 border border-white/[0.03] text-xs font-mono"
                    >
        ...
        {activeTab === 'languages' && (
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 max-w-4xl mx-auto">
            {LANGUAGES.map((lang, idx) => (
              <div
                key={idx}
                className="p-5 rounded-2xl bg-[#0c0e14] border border-white/[0.07] text-center space-y-3 hover:border-[#09C8FF]/30 transition-all"
              >
